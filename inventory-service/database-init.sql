...
SELECT 1, seat_number, seat_class, TRUE, price FROM (
...
INSERT INTO seats (flight_id, seat_number, seat_class, is_available, price) 
SELECT 2, seat_number, seat_class, TRUE, price FROM (
...
INSERT INTO seats (flight_id, seat_number, seat_class, is_available, price) 
SELECT 3, seat_number, seat_class, TRUE, price FROM (
...
-- 为其他航班也插入类似的座位数据...
-- 这里简化处理，只为前3个航班插入详细座位数据

-- 为了演示，我们可以将一些座位设置为已占用
